...
import { USE_MOCK, mockResponse, mockCurrentUser, mockUsers, mockToken } from '@/mock'
...
export const useUserStore = defineStore('user', () => {
  ...
  async function login(credentials) {
    try {
      // Mock 模式
      if (USE_MOCK) {
        const user = mockUsers.find(u => u.username === credentials.loginId)
        if (user && credentials.password === '123456') {
          ...
          departmentId.value = user.department_id || null
  ...
  async function initialize(admin) {
    try {
      // Mock 模式
      if (USE_MOCK) {
        const user = { ...mockCurrentUser, uid: admin.uid, phone_number: admin.phone_number }
        token.value = mockToken
        userId.value = user.id
        username.value = user.username
        uid.value = user.uid
