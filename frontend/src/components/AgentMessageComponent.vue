<template>
  ...
  <!-- 用户消息 -->
  <div v-if="message.type === 'human'" class="human-message-wrapper">
    <div class="message-box human">
      <p class="message-text">
        <MentionTextRenderer :content="message.content" :display-labels="mentionDisplayLabels" />
      </p>
    </div>
    <!-- 用户消息悬停操作按钮 -->
    <div class="human-message-actions">
      <span class="action-btn" @click="copyToClipboard(message.content)" title="复制">
        <Copy :size="14" />
      </span>
      <span class="action-btn" @click="emit('share', message)" title="转发">
        <Share2 :size="14" />
      </span>
      <span class="action-btn" @click="emit('edit', message)" title="编辑">
        <Pencil :size="14" />
      </span>
      <a-dropdown :trigger="['click']">
        ...
        <template #overlay>
          <a-menu>
            <a-menu-item key="retry" @click="emit('retry')">
              <RotateCcw :size="14" /> 重新发送
            </a-menu-item>
            <a-menu-item key="delete" @click="emit('delete', message)" class="delete-item">
              <Trash2 :size="14" /> 删除
            </a-menu-item>
          </a-menu>
        </template>
  ...
  >
    <p v-if="message.type === 'system'" class="message-text-system">{{ message.content }}</p>

    <!-- 助手消息内容 -->
    <div v-if="message.type === 'ai'" class="assistant-message">
