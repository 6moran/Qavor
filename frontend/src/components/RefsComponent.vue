<template>
  <div class="refs" v-if="showRefs">
    <div class="tags">
      ...
      <!-- 编辑 -->
      <span v-if="showKey('copy')" class="item btn" @click="emit('edit', msg)" title="编辑">
        <Pencil size="14" />
      </span>
      ...
      <!-- 删除 -->
      <span v-if="showKey('copy')" class="item btn" @click="emit('delete', msg)" title="删除">
        <Trash2 size="14" />
      </span>

      <!-- 模型名称 -->
      <span v-if="showKey('model') && getModelName(msg)" class="item model-item" @click="console.log(msg)">
...
const emit = defineEmits(['retry', 'openRefs', 'edit', 'delete'])
