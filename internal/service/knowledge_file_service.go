...
func (s *knowledgeFileService) RetryParse(ctx context.Context, kbID, fileID string) (*response.ProcessingJobEnqueueItem, error) {
	...
	if file.Status != entity.FileParseFailed && file.Status != entity.FileIndexed {
		return nil, bizerrors.New(bizerrors.CodeConflict, "只有解析失败或已入库的文件可以重新解析")
	}
	job := &entity.DocumentProcessingJob{
		JobID:       uuid.NewString(),
		KBID:        kbID,
	...
	created, err := s.jobRepo.CreateForFileTransition(ctx, job, []string{entity.FileParseFailed, entity.FileIndexed}, entity.FileParseQueued)
	if err != nil {
		return nil, err
	}
	if !created {
